...
data class LedgerPlayer(val uuid: UUID, val name: String, val firstJoin: Instant, val lastJoin: Instant, val passwordHash: String?, val webUiPerms: Byte) {
    ...
    companion object {
        fun fromRow(row: Tables.Player?): LedgerPlayer = LedgerPlayer(row!!.playerId, row.playerName, row.firstJoin, row.lastJoin, row.passwordHash, row.webUiPerms)
    }
}
