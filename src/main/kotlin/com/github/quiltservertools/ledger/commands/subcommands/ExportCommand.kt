...
object ExportCommand : BuildableCommand {
    override fun build(): LiteralNode {
        return literal("export")
            ...
            .then(SearchParamArgument.argument(CommandConsts.PARAMS)
                .executes {
    ...
    private fun run(source: ServerCommandSource, params: ActionSearchParams): Int {
        Ledger.launch {
            source.sendFeedback("text.ledger.export.started".translate().setStyle(TextColorPallet.primary), false)
            val results = DatabaseManager.searchActions(params, 0)
            ...
            source.sendFeedback(TranslatableText("text.ledger.export.actions", results.pages.toString().literal()
                .setStyle(TextColorPallet.secondaryVariant)).setStyle(TextColorPallet.secondary), false)
            ...
            var string = ""
            actions.forEach {
                string += "${it.getMessage().string}\n"
            }
            Files.createFile(path)
            Files.writeString(path, string)
            source.sendFeedback(TranslatableText("text.ledger.export.completed", path.pathString.literal()
                .setStyle(TextColorPallet.primaryVariant)).setStyle(TextColorPallet.primary), false)
            }
        return 1
    }
}
