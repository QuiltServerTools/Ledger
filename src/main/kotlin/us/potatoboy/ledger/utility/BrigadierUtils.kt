...
object BrigadierUtils {
     ...
     */
    fun <S> buildRedirect(
        alias: String, destination: LiteralCommandNode<S>
    ): LiteralCommandNode<S>? {
        // Redirects only work for nodes with children, but break the top argument-less command.
        // Manually adding the root command after setting the redirect doesn't fix it.
        // See https://github.com/Mojang/brigadier/issues/46). Manually clone the node instead.
        val builder = LiteralArgumentBuilder
            .literal<S>(alias.toLowerCase(Locale.ENGLISH))
            .requires(destination.requirement)
            .forward(
                destination.redirect, destination.redirectModifier, destination.isFork
            )
